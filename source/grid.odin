...
grid_init :: proc(grid: ^Grid) {
    ...
    for row_idx in 0 ..< GRID_SIZE_Y {
        for col_idx in 0 ..< GRID_SIZE_X {
            nx := (f32(col_idx) * 0.15) + seed_x
            ny := (f32(row_idx) * 0.15) + seed_y
    ...
    grid.tiles[GRID_SIZE_Y - 1][0].kind = .Medieval_SmallCastle
    grid.tiles[GRID_SIZE_Y - 1][0].team = .Player

    grid.tiles[0][GRID_SIZE_X - 1].kind = .Medieval_LargeCastle
    grid.tiles[0][GRID_SIZE_X - 1].team = .Enemy
}
